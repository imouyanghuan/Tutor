 ...
 *         2015-10-27
 */
public class TuitionCenterDetailActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.activity_tuition_center_detail);
		initData();
	}
	...
	protected void initView() {
		...
		bar.initBack(this);
		if (TutorApplication.getRole() == Constants.General.ROLE_TUITION_SCHOOL) {
			getView(R.id.ll_phone).setVisibility(View.GONE);
			getView(R.id.ll_address).setVisibility(View.GONE);
		}
		// view 如果之前有填寫則自動填寫上去
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			case R.id.btn_accept:
				if (TutorApplication.getRole() == Constants.General.ROLE_TUITION_SCHOOL) {
					acceptOrRejectForTuitionCenter(notificaiton.getId(), Constants.General.ACCEPT);
				} else {
			...
			case R.id.btn_reject:
				if (TutorApplication.getRole() == Constants.General.ROLE_TUITION_SCHOOL) {
					acceptOrRejectForTuitionCenter(notificaiton.getId(), Constants.General.REJECT);
				} else {
		...
		HttpHelper.getHelper().get(url, TutorApplication.getHeaders(), new RequestParams(), new ObjectHttpResponseHandler<EditProfileResult>(EditProfileResult.class) {
			...
			public void onSuccess(EditProfileResult result) {
				...
				if (result.getStatusCode() == 200) {
					Intent intent = new Intent();
					intent.putExtra(Constants.IntentExtra.INTENT_EXTRA_NOTIFICATION_ID, notificaiton.getId());
					intent.putExtra(Constants.IntentExtra.INTENT_EXTRA_NOTIFICATION_STATUS, status);
					setResult(Constants.RequestResultCode.NOTIFICATION_STATUS, intent);
					btnAccept.setEnabled(false);
					btnReject.setEnabled(false);
