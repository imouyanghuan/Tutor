 ...
 *         2015-9-18
 */
public class TuitionCenterNotificationAdapter extends BaseSwipeAdapter {

	private Context mContext;
	private List<Notification> mList;
	private ZSwipeItem swipeItem;

	public TuitionCenterNotificationAdapter(Context mContext, List<Notification> mList) {
	...
	public View generateView(int position, ViewGroup parent) {
		return View.inflate(mContext, R.layout.layout_tuition_center_notification_item, null);
	}

	@Override
	public void fillValues(final int position, View convertView) {
		ImageView imgAvatar = (ImageView) convertView.findViewById(R.id.student_list_item_avatar);
		TextView nickName = (TextView) convertView.findViewById(R.id.student_list_item_nick);
		TextView time = (TextView) convertView.findViewById(R.id.student_list_item_time);
		TextView msg = (TextView) convertView.findViewById(R.id.student_list_item_form);
		// TextView tvStatus = (TextView)
		// convertView.findViewById(R.id.student_list_item_form);
		...
		if (mList != null) {
			...
			if (status == Constants.General.REJECT || status == Constants.General.ACCEPT) {
		...
		if (TextUtils.isEmpty(msgFrom)) {
			nickName.setText("Tutor Student");
		} else {
			nickName.setText(msgFrom);
		}
